...
# 🔹 NEW: import the ADK error so we can handle duplicate sessions
...
weather_server_params = StdioServerParameters(
    command="python",
    args=[os.path.join(_current_dir, "mcp_server/weather_server.py")],
)
ct_server_params = StdioServerParameters(
    command="python",
    args=[os.path.join(_current_dir, "mcp_server/cocktail.py")],
)
...
async def websocket_endpoint(
    ...
    try:
        # 🔹 Try to create a new ADK session for this (app_name, user_id, session_id)
        try:
            ...
            # 🔹 If it already exists, just reuse it instead of crashing
            logging.info(
                f"ADK Session with id {session_id} already exists. Reusing existing session."
            )
